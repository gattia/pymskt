...
def pca_svd(data):
    ...
    data = (
        data - mn[:, None]
    )  # produce centered data. If already centered this shouldnt be harmful.
...
def get_ssm_deformation(PCs, Vs, mean_coords, pc=0, n_sds=3):
    ...
    pc_vector_scale = (
        np.sqrt(Vs[pc]) * n_sds
    )  # convert Variances to SDs & multiply by n_sds (negative/positive important)
        ...
        for idx, mesh_ in enumerate(mean_mesh):
            ...
            mesh.append(
                create_vtk_mesh_from_deformed_points_(
                    mesh_, new_points[start_idx : start_idx + (n_inputs * n_pts)], features=features
                )
            )
            start_idx += n_pts * n_inputs
    else:
        mesh = create_vtk_mesh_from_deformed_points_(mean_mesh, new_points, features=features)
    return mesh
...
def fit_ssm_to_mesh_least_squares(
    ...
    reconstructed_mesh_LS = Mesh(
        create_vtk_mesh_from_deformed_points(
            mean_mesh=ssm.mean_mesh.mesh, new_points=new_shape, features=ssm.vertex_features
        )
    )
...
def create_vtk_mesh_from_deformed_points_(
    mean_mesh, new_points, features=None, active_scalars=None
):
    """
