...
def break_cartilage_into_superficial_deep(bone_mesh, seg_image=None, list_cartilage_labels=None, rel_depth_thresh=0.5, resample_cartilage_surface=10_000, return_rel_depth=False, deep_label=100, superficial_label=200):
    """
    ...
    assert isinstance(seg_image, sitk.Image), f'seg_image is not a SimpleITK image: {type(seg_image)}'
    ...
    for surface in articular_surfaces:
        articular_cart_distances.append(abs(Mesh(surface).get_sdf_pts(voxel_coords_world, method='vtk')))
    ...
    new_seg_array = np.zeros_like(seg_arr, dtype=np.uint16) # set to uint16 to avoid overflow (many segs are int8 causing an issue)
    ...
    new_seg_array[superficial_idx[:, 0], superficial_idx[:, 1], superficial_idx[:, 2]] = superficial_label
