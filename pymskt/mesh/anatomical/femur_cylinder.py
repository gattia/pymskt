...
class FitCylinderFemur:
    ...
    def guess_height(self):
        ...
        height_guess = (
            self.inertial_aligned_pts_articular_cylinder[0, :].max()
            - self.inertial_aligned_pts_articular_cylinder[0, :].min()
        )
    ...
    def guess_origin(self):
        ...
        min_x = self.inertial_aligned_pts_articular_cylinder[
            0, :
        ].min()  # this is going to be fully medial or laterl
        max_x = self.inertial_aligned_pts_articular_cylinder[
            0, :
        ].max()  # this is going to be fully medial or laterl (opposite above)
        mean_y = self.inertial_aligned_pts_articular_cylinder[
            1, :
        ].mean()  # use this as the origin y
        mean_z = self.inertial_aligned_pts_articular_cylinder[
            2, :
        ].mean()  # I think this is going to be too close to the articular surface... but maybe good enought start?
    ...
    def guess_radius(self):
        ...
        range_y = (
            self.inertial_aligned_pts_articular_cylinder[1, :].max()
            - self.inertial_aligned_pts_articular_cylinder[1, :].min()
        )
        radius = range_y / 2
    ...
    def cylinder_function(
        self, origin, height, radius, vector, z_resolution=None, theta_resolution=None
    ):
        ...
        unit_cylinder = FitCylinderFemur.get_unit_cylinder(
            z_resolution=z_resolution, theta_resolution=theta_resolution
        )
    ...
    def fit(self):
        ...
        result = least_squares(func, self.params, bounds=self.bounds, ftol=self.ftol)
    ...
    def cylinder(self):
        return self.cylinder_function(
            origin=self._origin, height=self._height, radius=self._radius, vector=self._vector
        )
