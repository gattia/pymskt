...
from pymskt.image import crop_bone_based_on_width, read_nrrd
...
class Mesh(pv.PolyData):
    ...
    def get_assd(self, point_cloud):
        if isinstance(point_cloud, Mesh):
            ...
            raise TypeError(
                "point_cloud must be of type Mesh, vtk.vtkPolyData, or np.ndarray, and received: {}".format(
                    type(point_cloud)
                )
            )
    ...
    def orient_surface(self):
        ...
        """
        mesh_ = orient_surface(self.copy())
        self.deep_copy(mesh_)
    ...
    def boolean_operation(
        ...
        other_mesh : pyvista.PolyData or Mesh
            The other mesh to combine with
        op : str, optional
            Operation: 'union', 'intersection', 'difference', 'difference2'
    ...
    def boolean_difference(self, other_mesh, **kwargs):
        ...
        **kwargs : dict
        ...
        Requires vtkbool: `conda install -c conda-forge vtkbool`
        """
        return self.boolean_operation(other_mesh, op="difference", **kwargs)
    ...
    def boolean_intersection(self, other_mesh, **kwargs):
        ...
        **kwargs : dict
        ...
        Requires vtkbool: `conda install -c conda-forge vtkbool`
        """
        return self.boolean_operation(other_mesh, op="intersection", **kwargs)
    ...
    def copy_scalars_from_other_mesh_to_current(
        ...
        ** This function requires that the `other_mesh` is non-rigidly registered to the surface
        ...
        if orig_scalars_name is None:
            orig_scalars_name = [
                other_mesh.GetPointData().GetArray(array_idx).GetName()
                for array_idx in range(other_mesh.GetPointData().GetNumberOfArrays())
            ]
...
class CartilageMesh(Mesh):
    ...
    def __init__(
        self, mesh=None, seg_image=None, path_seg_image=None, label_idx=None, min_n_pixels=1000
    ):
        super().__init__(
            mesh=mesh,
            seg_image=seg_image,
...
class BoneMesh(Mesh):
    ...
    def create_cartilage_meshes(
        ...
        for cart_label_idx in self.list_cartilage_labels:
            ...
            if n_pixels_with_cart == 0:
                ...
                cart_mesh = CartilageMesh(seg_image=self._seg_image, label_idx=cart_label_idx)
                cart_mesh.create_mesh(
    ...
    def calc_cartilage_thickness(
        ...
        if list_cartilage_meshes is not None:
            self._list_cartilage_meshes = list_cartilage_meshes
        if list_cartilage_labels is not None:
            self._list_cartilage_labels = list_cartilage_labels
        ...
        if self._list_cartilage_meshes is None:
            self.create_cartilage_meshes(
                image_smooth_var_cart=image_smooth_var_cart,
                marching_cubes_threshold=marching_cubes_threshold,
            )
    ...
    def assign_cartilage_regions(
        ...
        vtk_seg_reader = read_nrrd(path_save_tmp_file, set_origin_zero=True)
        ...
        if isinstance(new_list_cartilage_labels, list):
            for label in new_list_cartilage_labels:
                if not isinstance(label, int):
                    raise TypeError(
                        f"Item in `list_cartilage_labels` is not a `int` - got {type(label)}"
                    )
