...
def calculate_t2_linear_fit(data_4d, tes, t2_cutoff=100, r2_cutoff=0.7):
    ...
    invalid_input_flat = invalid_input_mask.reshape(-1, num_echoes).any(axis=1) # Shape: [num_pixels]
    ...
    if np.any(valid_mask):
        ...
        sst = np.sum((log_data_valid - mean_log_signal_valid)**2, axis=0) # Shape [num_valid_pixels]
        ...
        r2_map_flat[final_valid_mask_indices] = r_squared[r2_cutoff_mask] # Assign the calculated r2 for valid pixels
...
class T2DataLoader:
    ...
    **kwargs : dict
    ...
    """
    SUPPORTED_INPUT_TYPES = {'dicom', 'parrec', 'nifti_series', 'nrrd_series', 'sitk_image', 'numpy_array'}
    ...
    def __init__(self, source, input_type, tes=None, **kwargs):
        ...
        self.round_decimals = kwargs.get('round_decimals', 3) # Relevant for DICOM/PARREC TE extraction
        ...
        if input_type == 'dicom':
            ...
            self._load_dicom(source, user_tes=tes)
             ...
             if not isinstance(source, str) or not source.lower().endswith('.par'):
                 raise ValueError("For 'parrec' input_type, 'source' must be a valid path to a .PAR file.")
             ...
             if not isinstance(source, list) or not all(isinstance(p, str) for p in source):
                 raise ValueError(f"For '{input_type}', 'source' must be a list of file paths (strings).")
             if not source:
             ...
             if not isinstance(source, list) or not all(isinstance(img, sitk.Image) for img in source):
                 raise ValueError("For 'sitk_image' input_type, 'source' must be a list of SimpleITK.Image objects.")
             if not source:
        ...
        if self._data_4d is not None and self._tes is not None:
             if self._data_4d.ndim != 4:
                 raise RuntimeError(f"Loaded data array has {self._data_4d.ndim} dimensions, expected 4.")
             if self._data_4d.shape[3] != len(self._tes):
                 raise RuntimeError(f"Number of echoes in data ({self._data_4d.shape[3]}) "
        ...
        if self._ref_image is not None and self._ref_image.GetDimension() == 4:
            ...
            self._ref_image = new_ref_image
             ...
             warnings.warn("Input was NumPy array without geometry info. Creating default reference image.")
    ...
    def _load_dicom(self, folder_path, user_tes):
        ...
        # 1. Get list of files
        try:
            list_filenames = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.lower().endswith('.dcm')]
        ...
        image_info_list = []
        unique_tes = set()
        unique_origins = set() # Using origin as a slice identifier might be more robust than SliceLocation
        ref_spacing = None
        ref_direction = None
        ...
        for filename in list_filenames:
            try:
                ...
                # Essential tags
                if not meta_reader.HasMetaDataKey(te_tag) or not meta_reader.HasMetaDataKey(ipp_tag):
                     warnings.warn(f"Skipping file {filename}: Missing required DICOM tag (EchoTime or ImagePositionPatient).")
                     continue
                ...
                if ref_spacing is None:
                    ...
                    if len(current_size) == 2: # 2D image
                        ...
                        slice_shape_rows_cols = (current_size[1], current_size[0]) # height, width
                         ...
                         warnings.warn(f"Skipping file {filename}: Unexpected image dimensions {current_size}.")
                         continue
                elif current_spacing != ref_spacing or current_direction != ref_direction:
                     warnings.warn(f"Skipping file {filename}: Inconsistent spacing or direction detected.")
                     continue
                # Check slice dimensions
                current_slice_shape = None
                if len(current_size) == 2: current_slice_shape = (current_size[1], current_size[0])
                elif len(current_size) == 3 and current_size[2] == 1: current_slice_shape = (current_size[1], current_size[0])
                if current_slice_shape != slice_shape_rows_cols:
                     warnings.warn(f"Skipping file {filename}: Inconsistent slice dimensions {current_slice_shape} vs {slice_shape_rows_cols}.")
                     continue
                ...
                try:
                    ...
                    image_info_list.append({
                     ...
                     warnings.warn(f"Skipping file {filename}: Could not parse TE '{te_str}' or origin '{ipp_str}'. Error: {e}")
                     continue
        ...
        if not image_info_list:
            raise IOError(f"Could not successfully read metadata from any DICOM files in {folder_path}.")
        if not unique_tes:
            raise ValueError("Could not extract any EchoTime values from DICOM metadata.")
        if not unique_origins:
             raise ValueError("Could not extract any ImagePositionPatient (origin) values from DICOM metadata.")
        ...
        for info in image_info_list:
            try:
                ...
                slice_array = sitk.GetArrayFromImage(slice_image) # Shape [1, rows, cols] or [rows, cols]
                ...
                if slice_array.ndim == 3 and slice_array.shape[0] == 1:
                    slice_array = slice_array.squeeze(axis=0)
                     ...
                     warnings.warn(f"Unexpected array dimension {slice_array.ndim} for file {info['filename']}. Skipping.")
                     continue

                # Check shape consistency again after loading pixels
                if slice_array.shape != (rows, cols):
                     warnings.warn(f"Pixel data shape mismatch {slice_array.shape} vs expected {(rows, cols)} for file {info['filename']}. Skipping.")
                     continue
                ...
                loaded_count += 1
                 ...
                 warnings.warn(f"Could not find index for origin {info['origin']} or TE {info['te']} for file {info['filename']}. This shouldn't happen.")
            except Exception as e:
                 warnings.warn(f"Error processing pixel data for file {info['filename']}: {e}")

        expected_count = num_slices * num_echoes
        if loaded_count < expected_count:
            warnings.warn(f"Successfully loaded pixel data for {loaded_count} slice/echo combinations, but expected {expected_count}. Array may have missing data.")
            # Should we raise an error here? Maybe depends on how critical completeness is.
            # For now, just warn.
        elif loaded_count > expected_count:
             warnings.warn(f"Loaded data for {loaded_count} slice/echo combinations, but expected only {expected_count}. Possible duplicate data?")
        ...
        ref_image_3d.SetSpacing(ref_spacing) # Assuming ref_spacing captured (x,y,z) or (x,y) correctly
        ...
        if user_tes is not None:
            ...
            if len(user_tes_arr) == num_echoes:
                ...
                warnings.warn(f"User-provided TEs length ({len(user_tes_arr)}) does not match number of echoes ({num_echoes}). Ignoring user TEs.")
    ...
    def _load_parrec(self, par_path, user_tes):
        ...
        if nib_image.ndim != 4:
            raise ValueError(f"PAR/REC file must be 4-dimensional. Found {nib_image.ndim} dimensions in {par_path}")
        ...
        try:
            ...
            # after potentially removing TE=0 and applying drop_echo_1
             ...
             raise ValueError(f"Could not extract echo times from PAR header in {par_path}: {e}") from e
        except Exception as e:
             raise ValueError(f"Error processing echo times from PAR header in {par_path}: {e}") from e
        ...
        if user_tes is not None:
            ...
            if len(user_tes_arr) == len(extracted_tes):
                ...
                warnings.warn(f"User-provided TEs length ({len(user_tes_arr)}) does not match number of echoes ({len(extracted_tes)}) extracted from PAR header. Using extracted TEs.")
        ...
        if len(zero_te_indices) > 1:
             warnings.warn(f"Found multiple ({len(zero_te_indices)}) zero echo times in {par_path}. This is unexpected. Using the first encountered.")
        ...
        if len(valid_tes_list) == 0:
            raise ValueError(f"No non-zero echo times found after processing PAR/REC file {par_path}")
        ...
        valid_original_indices_of_min_te = [idx for idx in original_indices_of_min_te if idx in valid_echo_indices]

        if not valid_original_indices_of_min_te:
             raise ValueError(f"Could not locate the minimum TE ({min_te_val}) among valid echo indices.")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            nib_first_echo = nib.Nifti1Image(first_echo_array, affine=nib_image.affine, header=nib_image.header)
            ...
            try:
                 ...
                 ref_sitk_image.SetMetaData("intent_code", "") # Clear potential intent code if needed
                 ...
                 raise IOError(f"Failed to read temporary NIfTI file {tmp_nii_filename} with SimpleITK: {e}") from e
            ...
            if vendor_map_idx is not None:
                ...
                nib_vendor_map = nib.Nifti1Image(vendor_map_array, affine=nib_image.affine, header=nib_image.header)
                ...
                try:
                    ...
                    warnings.warn(f"Failed to read temporary vendor map NIfTI file {tmp_vendor_nii}: {e}")
            ...
            if not (nib_x_dim == sitk_x_dim and nib_y_dim == sitk_y_dim and nib_z_dim == sitk_z_dim):
                 warnings.warn("Dimension mismatch between nibabel array ({nib_x_dim},{nib_y_dim},{nib_z_dim}) "
            ...
            for i, original_idx in enumerate(valid_echo_indices):
                ...
                if echo_sitk_zyx.shape != ref_shape_3d:
                    raise ValueError(f"Shape mismatch after transpose for echo index {original_idx}. "
        ...
        if self.drop_echo_1:
            if len(self._tes) <= 1:
                 warnings.warn("drop_echo_1=True but only one echo remains after processing. Cannot drop.")
            else:
                ...
                warnings.warn("Dropped the first echo (lowest TE) as requested by drop_echo_1=True.")
    ...
    def _load_file_series(self, filepaths, tes):
        ...
        if not isinstance(tes, (list, np.ndarray)):
            raise ValueError("'tes' must be provided as a list or NumPy array for file series loading.")
        tes_array = np.asarray(tes)
        if len(tes_array) != len(filepaths):
            raise ValueError(f"Number of provided TEs ({len(tes_array)}) does not match number "
    ...
    def _process_image_list(self, image_list, tes):
        ...
        if len(tes_array) != len(image_list):
             raise ValueError(f"Mismatch between number of TEs ({len(tes_array)}) and images ({len(image_list)}).")
        ...
        for i, image in enumerate(sorted_image_list):
            try:
                if image.GetDimension() != 3:
                    raise ValueError(f"Image at index {i} (sorted) is not 3D (Dimension: {image.GetDimension()}).")
                ...
                if i == 0:
                    ...
                    if geom_mismatch:
                        raise ValueError(f"Geometric mismatch in image at index {i} (sorted). Differs from first image in: {', '.join(mismatch_reason)}.")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to stack image arrays into 4D: {e}. Ensure all images have identical dimensions.") from e
...
def calculate_t2_nonlinear_fit(data_4d, tes, p0_maps=None, t2_cutoff=100, r2_cutoff=0.8, mask=None):
    ...
    if p0_maps is None or not isinstance(p0_maps, dict) or 't2' not in p0_maps or 'pd' not in p0_maps:
        raise ValueError("p0_maps dictionary with 't2' and 'pd' keys (from linear fit) is required for non-linear fit.")
    ...
    if num_pixels_to_fit == 0:
        warnings.warn("No valid pixels found to perform non-linear fitting based on p0 maps and mask.")
    ...
    for i in range(num_pixels_to_fit):
        ...
        try:
            ...
            if ss_tot < np.finfo(float).eps: # Avoid division by zero if data is constant
                ...
                if r_squared < 0: r_squared = 0 # Clamp R2 >= 0
...
def calculate_t2_map(source, input_type, method='nonlinear', tes=None, **kwargs):
    ...
    **kwargs :
    ...
    try:
        linear_results = calculate_t2_linear_fit(data_4d, loaded_tes,
                                                 t2_cutoff=linear_t2_cutoff,
                                                 r2_cutoff=linear_r2_cutoff)
    except Exception as e:
