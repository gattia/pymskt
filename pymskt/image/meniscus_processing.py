...
def _apply_plane_split(seg_array, original_meniscus_mask, ml_axis, center_split, med_side_is_positive, verbose=False):
    """Applies a splitting plane to separate original meniscus voxels."""
    if verbose: print(f"Applying plane split at axis {ml_axis} = {center_split}")
...
def verify_and_correct_meniscus_sides(
    ...
    if meniscus_coords.shape[0] == 0:
        ...
        return (np.zeros_like(seg_array, dtype=bool),
                np.zeros_like(seg_array, dtype=bool),
                None)
    ...
    if med_cart_valid:
        ...
        med_cart_centroid_3d = np.array(med_cart_locs).mean(axis=1) # Order Z, Y, X ? Check numpy order
        if verbose: print(f"Medial Tibial Cartilage Centroid (ML-axis): {middle_med_cart:.2f}")
    if lat_cart_valid:
        middle_lat_cart = lat_cart_locs[ml_axis].mean()
        lat_cart_centroid_3d = np.array(lat_cart_locs).mean(axis=1)
        if verbose: print(f"Lateral Tibial Cartilage Centroid (ML-axis): {middle_lat_cart:.2f}")
    ...
    if med_cart_valid and lat_cart_valid:
        ...
        if med_direction == lat_direction:
            raise ValueError("Medial and lateral tibial cartilage centroids are on the same side of the tibial bone centroid. Cannot use 'cartilage_midpoint'.")
        cartilage_contradictory = False
        med_side_is_positive = med_direction > 0
    elif med_cart_valid or lat_cart_valid:
         cartilage_contradictory = False
         if med_cart_valid: med_side_is_positive = np.sign(middle_med_cart - middle_tib_bone) > 0
         else: med_side_is_positive = np.sign(middle_lat_cart - middle_tib_bone) < 0
         if verbose:
             print("Using only one valid tibial cartilage label to determine medial/lateral direction.")
             print("Consider using 'tibial_centroid' method instead, which does not require valid cartilage labels.")
    else:
         cartilage_contradictory = False # No cartilage to contradict
         med_side_is_positive = None # Cannot determine direction
         if verbose: print("No valid tibial cartilage found to determine medial/lateral direction.")
    ...
    if method == 'tibial_centroid':
        ...
        if verbose: print(f"Using Tibial Bone Centroid Split Point: {center_split}")
        if med_side_is_positive is None:
            raise ValueError("Cannot determine Medial direction for tibial_centroid method without valid cartilage.")
        ...
        if not med_cart_valid or not lat_cart_valid:
            raise ValueError("Method 'cartilage_midpoint' requires valid medial and lateral tibial cartilage labels.")
        if cartilage_contradictory:
            raise ValueError("Tibial cartilage centroids are on the same side. Cannot use 'cartilage_midpoint'.")
        center_split = int(np.round((middle_med_cart + middle_lat_cart) / 2.0))
        if verbose: print(f"Using Tibial Cartilage Midpoint Split Point: {center_split}")
        ...
        if not med_cart_valid or not lat_cart_valid:
             raise ValueError("Method 'cartilage_distance' requires valid medial and lateral tibial cartilage labels.")
        if cartilage_contradictory:
             raise ValueError("Tibial cartilage centroids are on the same side. Cannot use 'cartilage_distance'.")

        if verbose: print(f"Using distance to 3D Tibial Cartilage Centroids: Med={med_cart_centroid_3d}, Lat={lat_cart_centroid_3d}")

        # --- Incorporate Spacing --- check spacing_zyx validity
        if not isinstance(spacing_zyx, (tuple, list)) or len(spacing_zyx) != 3 or not all(isinstance(x, (int, float)) and x > 0 for x in spacing_zyx):
            warnings.warn(f"Invalid spacing_zyx {spacing_zyx}; assuming isotropic voxels (1.0, 1.0, 1.0). Expected tuple/list of 3 positive numbers.")
            spacing_zyx = (1.0, 1.0, 1.0)
        # Add specific warning if default isotropic spacing is being used
        if tuple(spacing_zyx) == (1.0, 1.0, 1.0):
             warnings.warn("Using default spacing (1.0, 1.0, 1.0) for 'cartilage_distance'. Results may be inaccurate for anisotropic voxels. Provide correct 'spacing_zyx'.")

        spacing_array = np.array(spacing_zyx)
        if verbose: print(f"Using spacing (ZYX): {spacing_array}")
        # --------------------------
        ...
        if not _SCIPY_AVAILABLE: raise ImportError("Method 'logistic_meniscus' requires scipy.optimize.")
        if verbose: print("Using Logistic Regression on Meniscus ML coordinates.")
        ...
        result = minimize(_cost_function, theta, args=(X, meniscus_labels_binary),
                          method='BFGS', jac=_gradient, options={'maxiter': 400, 'disp': verbose})
        if not result.success: raise RuntimeError(f"Logistic regression failed to converge: {result.message}")
        theta_optimized = result.x

        if np.abs(theta_optimized[1]) < 1e-6: # Coefficient is near zero -> Fallback
            warnings.warn("Logistic regression ML coefficient is near zero. Falling back to tibial centroid split.")
            center_split = int(np.round(middle_tib_bone))
            if verbose: print(f"Using Fallback Tibial Bone Centroid Split Point: {center_split}")
            if med_side_is_positive is None:
                 raise ValueError("Cannot determine Medial direction for logistic fallback without valid cartilage.")
            cleaned_med_men_mask, cleaned_lat_men_mask = _apply_plane_split(
                seg_array, original_meniscus_mask, ml_axis, center_split, med_side_is_positive, verbose
            )
        else: # Logistic regression succeeded
            center_split = -theta_optimized[0] / theta_optimized[1]
            if verbose: print(f"Logistic Regression Boundary Split Point: {center_split:.2f}")
            center_split = int(np.round(center_split))
            logistic_med_side_is_positive = theta_optimized[1] < 0
            if med_side_is_positive is not None and med_side_is_positive != logistic_med_side_is_positive:
                warnings.warn("Logistic regression direction disagrees with cartilage-derived direction. Using logistic result.")
            med_side_is_positive = logistic_med_side_is_positive # Use logistic result for direction
            cleaned_med_men_mask, cleaned_lat_men_mask = _apply_plane_split(
                seg_array, original_meniscus_mask, ml_axis, center_split, med_side_is_positive, verbose
            )
...
def subdivide_meniscus_regions(
    ...
    output_array[~np.isin(seg_array, meniscus_labels)] = seg_array[~np.isin(seg_array, meniscus_labels)]
    ...
    try:
        ...
        cleaned_med_men_mask, cleaned_lat_men_mask, center_split = verify_and_correct_meniscus_sides(
    ...
    if verbose:
        print(f"Using spacing: ML={spacing_ml:.4f} (axis {ml_axis}), AP={spacing_ap:.4f} (axis {ap_axis})")
    ...
    tib_locs = np.where(seg_array == tib_label)
    if tib_locs[0].size == 0:
         raise ValueError(f"Tibial bone label ({tib_label}) not found after cleanup check.") # Should not happen if helper ran

    tib_center_ml = tib_locs[ml_axis].mean()
    tib_center_ap = tib_locs[ap_axis].mean()
    if verbose: print(f"Tibial Centroid (ML, AP): ({tib_center_ml:.2f}, {tib_center_ap:.2f})")
    ...
    if center_method == 'tibia':
        ...
        if verbose: print("Using global tibial centroid as polar center.")
    elif center_method == 'cartilage':
        if verbose: print("Using individual tibial cartilage centroids as polar centers.")
        med_cart_locs = np.where(seg_array == med_tib_cart_label)
        lat_cart_locs = np.where(seg_array == lat_tib_cart_label)

        if med_cart_locs[0].size < min_cart_voxels:
             raise ValueError(f"Medial tibial cartilage label {med_tib_cart_label} has insufficient voxels ({med_cart_locs[0].size} < {min_cart_voxels}) for center_method='cartilage'.")
        if lat_cart_locs[0].size < min_cart_voxels:
             raise ValueError(f"Lateral tibial cartilage label {lat_tib_cart_label} has insufficient voxels ({lat_cart_locs[0].size} < {min_cart_voxels}) for center_method='cartilage'.")
        ...
        if verbose: print(f"Medial Cartilage Center (ML, AP): ({med_center_ml:.2f}, {med_center_ap:.2f})")
        if verbose: print(f"Lateral Cartilage Center (ML, AP): ({lat_center_ml:.2f}, {lat_center_ap:.2f})")

        # Optionally adjust centers inward along the line connecting them
        if cartilage_center_offset_fraction > 0:
             if verbose: print(f"Adjusting cartilage centers inward by fraction: {cartilage_center_offset_fraction}")
             ...
             if verbose:
                 print(f"Adjusted Medial Center (ML, AP): ({med_center_ml:.2f}, {med_center_ap:.2f})")
                 print(f"Adjusted Lateral Center (ML, AP): ({lat_center_ml:.2f}, {lat_center_ap:.2f})")

    else:
        raise ValueError(f"Invalid center_method: '{center_method}'. Choose 'tibia' or 'cartilage'.")

    # --- Determine Anterior Reference Angle (if using patella) ---
    anterior_reference_angle_rad = None
    if verbose: print("Using patella centroid to define Anterior direction.")
    ...
    if pat_label is not None and np.any(seg_array == pat_label):
        ...
        if verbose: print(f"Using patella bone label {pat_label} for anchor.")
    elif pat_cart_label is not None and np.any(seg_array == pat_cart_label):
        pat_anchor_label = pat_cart_label
        if verbose: print(f"Using patellar cartilage label {pat_cart_label} for anchor.")
    else:
        raise ValueError("This function requires either pat_label or pat_cart_label to be provided and present in the segmentation to define the Anterior direction.")

    pat_locs = np.where(seg_array == pat_anchor_label)
    pat_center_ml = pat_locs[ml_axis].mean()
    pat_center_ap = pat_locs[ap_axis].mean()
    if verbose: print(f"Patellar Anchor Centroid (ML, AP): ({pat_center_ml:.2f}, {pat_center_ap:.2f})")
    ...
    if verbose: print(f"Anterior Reference Angle (Patella direction = 0 degrees): {np.degrees(anterior_reference_angle_rad):.2f} degrees relative to ML axis.")

    # --- Classify Voxels into Regions using Helper Function ---
    if verbose: print("Classifying meniscus voxels into Anterior/Middle/Posterior regions...")
    ...
    if center_method == 'cartilage':
        if n_med > 0:
            med_is_anterior, med_is_middle, med_is_posterior = \
                _classify_voxel_regions_by_angle(med_men_indices, med_center_ml, med_center_ap, 
                                               anterior_reference_angle_rad, ml_axis, ap_axis, spacing_ml, spacing_ap)
        if n_lat > 0:
            lat_is_anterior, lat_is_middle, lat_is_posterior = \
                _classify_voxel_regions_by_angle(lat_men_indices, lat_center_ml, lat_center_ap, 
                                               anterior_reference_angle_rad, ml_axis, ap_axis, spacing_ml, spacing_ap)
        if verbose: print("Region classification complete using cartilage centers.")

    else: # center_method == 'tibia'
        all_men_indices = np.vstack((med_men_indices, lat_men_indices))
        if all_men_indices.shape[0] > 0:
            is_anterior, is_middle, is_posterior = \
                _classify_voxel_regions_by_angle(all_men_indices, tib_center_ml, tib_center_ap, 
                                               anterior_reference_angle_rad, ml_axis, ap_axis, spacing_ml, spacing_ap)
        ...
        if verbose: print("Region classification complete using tibial center.")

    # --- Assign Sub-regions ---
    if verbose: print("Assigning voxels to sub-regions...")
    ...
    if n_med > 0:
        ...
        if np.any(unassigned_med):
             warnings.warn(f"{np.sum(unassigned_med)} medial meniscus voxels were not assigned a sub-region label. Check angle range definitions.")
    ...
    if n_lat > 0:
        ...
        if np.any(unassigned_lat):
             warnings.warn(f"{np.sum(unassigned_lat)} lateral meniscus voxels were not assigned a sub-region label. Check angle range definitions.")
    ...
    # --- Create Output Image ---
    if verbose: print("Creating final output image...")
    ...
    if verbose: print("Meniscus sub-region division complete.")
