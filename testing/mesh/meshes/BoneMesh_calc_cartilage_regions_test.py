...
def test_if_calculated_cartilage_region_matches_saved_file(
    bone_mesh=BONE_MESH, cartilage_mesh=CARTILAGE_MESH, ref_mesh=REF_MESH, seg_image=SEG_IMAGE
):
    cart_mesh = mskt.mesh.CartilageMesh(mesh=cartilage_mesh)

    mesh = mskt.mesh.BoneMesh(
        mesh=bone_mesh, list_cartilage_meshes=[cart_mesh], seg_image=seg_image
    )
