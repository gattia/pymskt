...
def prepare_sitk_test_data_from_dicom(dicom_folder, output_folder, file_format='nii.gz'):
    ...
    if len(tes) != num_echoes:
        raise RuntimeError(f"Mismatch between number of TEs ({len(tes)}) and data dimension ({num_echoes}).")
...
def test_t2dataloader_load_dicom():
    ...
    expected_ref_direction = (
        -0.02183430508768329, -0.14477205880584315, -0.9892240970126311,
         0.9995105915952667,  0.019010007721790664, -0.02484344773371071,
         0.022401794799036173, -0.9892824018426362,  0.14428613583517622
    )
...
def test_t2dataloader_parrec():
    ...
    assert data_array.shape == expected_shape_zyxt, \
    ...
    assert len(tes) == data_array.shape[3], "Number of TEs should match the 4th dimension of the data array"
    # Specific TE value check
    expected_tes = np.array([6.36, 12.72, 19.09, 25.45, 31.81, 38.17, 44.53, 50.90, 57.26, 63.62, 69.98, 76.34])
    assert np.allclose(tes, expected_tes, rtol=1e-3), \
    ...
    assert ref_image.GetSize() == expected_size_xyz, \
    ...
    assert np.allclose(ref_image.GetSpacing(), expected_spacing_xyz, rtol=1e-3), \
        f"Reference image spacing {ref_image.GetSpacing()} does not match expected {expected_spacing_xyz}"

    # Test with drop_echo_1=True
    try:
        loader_drop1 = t2.T2DataLoader(source=PARREC_TEST_DATA_PATH, input_type='parrec', drop_echo_1=True)
    except Exception as e:
    ...
    assert data_array_drop1.shape[3] == data_array.shape[3] - 1, "Data array should have one less echo after drop_echo_1"
...
def test_t2dataloader_sitk_list(mese_sitk_series_data):
    ...
    assert isinstance(data_array, np.ndarray), "Data array should be a NumPy array"
    assert data_array.ndim == 4, f"Data array should be 4D, but got {data_array.ndim}D"
    assert data_array.shape[3] == len(images), "Number of echoes in array must match number of input images"
    ...
    assert len(loaded_tes) == data_array.shape[3], "Number of loaded TEs should match data array echo dimension"
    ...
    assert np.allclose(ref_image.GetSpacing(), first_image.GetSpacing()), "Reference image spacing mismatch"
    assert np.allclose(ref_image.GetOrigin(), first_image.GetOrigin()), "Reference image origin mismatch"
    assert np.allclose(ref_image.GetDirection(), first_image.GetDirection()), "Reference image direction mismatch"

    # Test with drop_echo_1=True
    try:
        # Use drop_echo_1=True
        loader_drop1 = t2.T2DataLoader(source=images, input_type='sitk_image', tes=tes, drop_echo_1=True)
    except Exception as e:
        pytest.fail(f"T2DataLoader(sitk_image list) initialization with drop_echo_1=True failed: {e}")

    data_array_drop1 = loader_drop1.get_data_array()
    tes_drop1 = loader_drop1.get_tes()

    assert data_array_drop1.shape[3] == data_array.shape[3] - 1, "Data array should have one less echo after drop_echo_1 (sitk_list)"
    assert len(tes_drop1) == len(loaded_tes) - 1, "Should have one less TE after drop_echo_1 (sitk_list)"
    assert np.min(tes_drop1) > np.min(loaded_tes), "The minimum TE should be higher after drop_echo_1 (sitk_list)"
