...
def test_calculate_t2_linear_fit_basic():
    ...
    tes = np.array([10., 25., 40., 55., 70.])

    data_4d, tes, _, _ = generate_synthetic_t2_data(shape=shape, tes=tes, pds=pds, t2s=t2s, noise_level=0.0)
    ...
    assert_allclose(results['t2'], true_t2, rtol=1e-5, atol=1e-5, err_msg="T2 map does not match expected value")
    assert_allclose(results['pd'], true_pd, rtol=1e-5, atol=1e-5, err_msg="PD map does not match expected value")
    assert_allclose(results['r2'], 1.0, rtol=1e-5, atol=1e-5, err_msg="R2 map should be close to 1.0 for perfect fit")
...
def test_calculate_t2_linear_fit_cutoffs():
    ...
    assert_allclose(results['t2'][0, 0, 1], 0.0, atol=1e-9, err_msg="Voxel (0,0,1) T2 should be 0 due to t2_cutoff")
    assert_allclose(results['pd'][0, 0, 1], 0.0, atol=1e-9, err_msg="Voxel (0,0,1) PD should be 0 due to t2_cutoff") # PD also zeroed if T2 is
    assert_allclose(results['r2'][0, 0, 1], 0.0, atol=1e-9, err_msg="Voxel (0,0,1) R2 should be 0 due to t2_cutoff") # R2 also zeroed

    assert results['r2'][0, 1, 0] < r2_cutoff, "Voxel (0,1,0) R2 should be low due to noise" # Check R2 is actually low first
    assert_allclose(results['t2'][0, 1, 0], 0.0, atol=1e-9, err_msg="Voxel (0,1,0) T2 should be 0 due to r2_cutoff")
    assert_allclose(results['pd'][0, 1, 0], 0.0, atol=1e-9, err_msg="Voxel (0,1,0) PD should be 0 due to r2_cutoff")
    ...
    assert_allclose(results['r2'][0, 1, 0], 0.0, atol=1e-9, err_msg="Voxel (0,1,0) R2 should be 0 because T2 was zeroed by r2_cutoff")
...
def test_calculate_t2_nonlinear_fit_basic():
    ...
    # Set high cutoffs
    results = t2.calculate_t2_nonlinear_fit(data_4d, tes, p0_maps=p0_maps, t2_cutoff=500, r2_cutoff=0.95)

    # Check results - expect near perfect fit for noiseless data
    # Non-linear fit might be slightly less precise than linear algebra for perfect data
    assert_allclose(results['t2'], true_t2, rtol=1e-4, atol=1e-4, err_msg="NL T2 map does not match expected value")
    assert_allclose(results['pd'], true_pd, rtol=1e-4, atol=1e-4, err_msg="NL PD map does not match expected value")
    assert_allclose(results['r2'], 1.0, rtol=1e-4, atol=1e-4, err_msg="NL R2 map should be close to 1.0 for perfect fit")
...
def test_calculate_t2_nonlinear_fit_cutoffs():
    ...
    results = t2.calculate_t2_nonlinear_fit(data_4d, tes, p0_maps=p0_maps, t2_cutoff=t2_cutoff, r2_cutoff=r2_cutoff)
    ...
    assert_allclose(results['t2'][0, 0, 1], 0.0, atol=1e-9, err_msg="NL Voxel (0,0,1) T2 should be 0 due to t2_cutoff")
    assert_allclose(results['pd'][0, 0, 1], 0.0, atol=1e-9, err_msg="NL Voxel (0,0,1) PD should be 0 due to t2_cutoff")
    assert_allclose(results['r2'][0, 0, 1], 0.0, atol=1e-9, err_msg="NL Voxel (0,0,1) R2 should be 0 due to t2_cutoff")

    # Check R2 value for the modified voxel before asserting T2/PD are zeroed
    # A constant signal should ideally yield R2=0
    assert results['r2'][0, 1, 0] < r2_cutoff, f"NL Voxel (0,1,0) R2 ({results['r2'][0, 1, 0]:.4f}) should be low (constant signal)"
    assert_allclose(results['t2'][0, 1, 0], 0.0, atol=1e-9, err_msg="NL Voxel (0,1,0) T2 should be 0 due to r2_cutoff")
    assert_allclose(results['pd'][0, 1, 0], 0.0, atol=1e-9, err_msg="NL Voxel (0,1,0) PD should be 0 due to r2_cutoff")
    assert_allclose(results['r2'][0, 1, 0], 0.0, atol=1e-9, err_msg="NL Voxel (0,1,0) R2 should be 0 because fit was rejected by r2_cutoff")
...
def test_calculate_t2_nonlinear_fit_masking():
    ...
    results = t2.calculate_t2_nonlinear_fit(data_4d, tes, p0_maps=p0_maps, mask=mask, t2_cutoff=200, r2_cutoff=0.9)
...
def test_calculate_t2_nonlinear_fit_input_validation():
    ...
    with pytest.raises(ValueError, match="p0_maps\['t2'\] must be a NumPy array with shape"):
        t2.calculate_t2_nonlinear_fit(data_4d_correct, tes, p0_maps={'t2': p0_t2_wrong_shape, 'pd': p0_pd_correct})
    ...
    with pytest.raises(ValueError, match="mask must be a boolean NumPy array with shape"):
        t2.calculate_t2_nonlinear_fit(data_4d_correct, tes, p0_maps=p0_maps_correct, mask=mask_wrong_shape)

    # Incorrect mask dtype
    mask_wrong_dtype = np.random.rand(*shape_3d)
    with pytest.raises(ValueError, match="mask must be a boolean NumPy array with shape"):
        t2.calculate_t2_nonlinear_fit(data_4d_correct, tes, p0_maps=p0_maps_correct, mask=mask_wrong_dtype)
...
@pytest.mark.skip(reason="Test for calculate_t2_map with PAR/REC input slow right now... not sure why... maybe too much data? Or maybe the mask is not working?.")
...
def test_calculate_t2_map_parrec():
    ...
    try:
        ...
        resampled_seg_image = sitk.Resample(seg_image, ref_image, transform, sitk.sitkNearestNeighbor)
        ...
        assert mask_array.shape == expected_mask_shape, \
    ...
    # --- Run Linear Fit (ONCE) ---
    try:
        linear_results_np = t2.calculate_t2_linear_fit(data_4d, loaded_tes,
                                                      t2_cutoff=100, # Provide default directly
                                                      r2_cutoff=0.7) # Provide default directly
    except Exception as e:
    ...
    for map_name, map_image in linear_maps_sitk.items():
        ...
        assert np.allclose(map_image.GetSpacing(), expected_spacing_xyz, rtol=1e-3), \
    ...
    try:
        nonlinear_results_np = t2.calculate_t2_nonlinear_fit(data_4d, loaded_tes,
